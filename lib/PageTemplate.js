...
class PageTemplate {
    ...
    footerHTML() {
        const dateCreated = fs.statSync('lib/server.js').birthtime.toISOString().slice(0, 4);
        const currentYear = (new Date()).getFullYear();
        const displayYear = currentYear > dateCreated ? `${dateCreated}&nbsp-&nbsp${currentYear}` : `${dateCreated}`
        return `<footer class="container">
                    <div class="row main-footer">
                        &copy;${displayYear}&nbsp;-&nbsp;<a href="https://github.com/eyyMinda" target="_blank">Mindaugas</a>
                    </div>
                </footer>`;
    }
